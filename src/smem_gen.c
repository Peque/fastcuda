...
void print_we_safe()
{
	...
	for (i=0;i<N;i++) {
		for (j=3;j>=0;j--) {
			printf("	we_%d_safe(%d) <= WE_%d(%d) and to_stdulogic(k%d_needs_attention);\n", i, j, i, j, i);
		}
		printf("\n");
	}
}
...
void print_needs_attention()
{
	int i, j;

	for (i=0;i<N;i++) {
		printf("	k%d_needs_attention <= to_bit(REQ_%d)", i, i);
		for (j=0;j<i;j++) {
			printf(" and (not (addr_%d_eq_addr_%d and k%d_needs_attention))", j, i, j);
		}
		printf(";\n\n");
	}
}
...
void print_needs_bram()
{
	...
	for (i=0;i<N;i++) {
		for (j=0;j<N/2;j++) {
			printf("	k%d_needs_bram_%d <= k%d_needs_attention", i, j, i);
...
void print_addr_eq()
{
	...
	for (i=0; i<N-1; i++) {
		for (j=i+1; j<N; j++) {
			printf("	addr_%d_eq_addr_%d <= ( ", i, j);
			for (k=(int) log2(N) + 7; k>=0; k--) {
				if (k<log2(N) + 7) printf("	                      ");
				printf("(to_bit(ADDR_%d(%d)) xnor to_bit(ADDR_%d(%d)))", i, k, j, k);
				if (k) printf(" and\n");
				else printf(" );\n\n");
			}
		}
...
void print_being_served()
{
	...
	for (i=0; i<N; i++) {
		...
		if (i != 0 && i != N - 1) {
			...
			printf("	                       (not k%d_needs_attention) )", i);
		}
		printf(";\n\n");
	}
}
...
void print_arch()
{
	...
	printf("\n");
	for (i=0;i<N;i++) {
		printf("	signal k%d_needs_attention  : bit := '0';\n", i);
	}
	printf("\n");
	for (i=0;i<N;i++) {
		for (j=i+1;j<N;j++) {
			printf("	signal addr_%d_eq_addr_%d    : bit := '0';\n", i, j);
		}
	}
	printf("\n");
	for (i=0;i<N;i++) {
